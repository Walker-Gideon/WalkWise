import { LuChevronsRight, LuRefreshCw } from "react-icons/lu";

import Button from "/src/ui/Button";
import HeaderText from "/src/ui/HeaderText";

import { useChat } from "/src/contexts/useChat.js";

export default function ChatHeader() {
  const { closeChat, refreshChat, isRefreshing } = useChat();

  return (
    <header className="mb-4 flex h-10 w-full items-center justify-between">
      <div className="flex items-center space-x-1">
        <Button variant="text" onClick={closeChat}>
          <LuChevronsRight className="h-6 w-6 text-slate-400 dark:text-slate-500" />
        </Button>
        <HeaderText type="secondary">Study Assistant</HeaderText>
      </div>

      <Button
        disabled={isRefreshing}
        variant="text"
        classname="disabled:cursor-not-allowed"
        onClick={refreshChat}
      >
        <LuRefreshCw className="h-5 w-5 text-slate-400 dark:text-slate-500" />
      </Button>
    </header>
  );
}
